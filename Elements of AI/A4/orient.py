...
if __name__== "__main__":
    ...
    if sys.argv[4] == 'nearest':
        if sys.argv[1] == 'train':
            data = pd.read_table(sys.argv[2], delim_whitespace=True, header=None)
            data.to_csv(sys.argv[3], header=None, index=None, sep=' ', mode='a')

            
        if sys.argv[1] == 'test':
            test = pd.read_table(sys.argv[2], delim_whitespace=True, header=None)
            train = pd.read_table(sys.argv[3], delim_whitespace=True, header=None)
            pred, accuracy = knn(train,test)
            print("Classification accuracy")
            print(accuracy*100)
            file = open("pred_file_knn.txt", 'a')
    ...
    if sys.argv[4] == 'tree':
        if sys.argv[1] == 'train':
            data = pd.read_table(sys.argv[2], delim_whitespace=True, header=None)
            key, data1 = process_data_dt(data)
    ...
    if sys.argv[4] == 'nnet':
        if sys.argv[1] == 'train':
            new = Neural_Net([100], 0.3, 7, sys.argv[2])
            new.train()
            new.save_params(sys.argv[3])

        if sys.argv[1] == 'test':
            new = Neural_Net([100], 0.3, 7, sys.argv[2])
            new.extract_params(sys.argv[3])
            new.check_test()
