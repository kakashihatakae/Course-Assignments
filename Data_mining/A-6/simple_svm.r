...
#  minimize x^t Q x + q^t x s.t. A x >= b
# where 
#  x is a p-vector 
#  Q is pxp matrix
#  q is p-vector
...
#
# Q is 0-1 matrix with only the first p (2 in this case) diagonal elements 1
# q is 0
# The first two columns of A are the X matrix with sign flips according to y
# and the last column is y
# b is a vector of 1's

# construct the corresponding elements: Q,q, A, b

Q = diag(3);
Q[3,3] = .001;  # first two components are the w elements and last one b
# minor kludge here,  need a positive definite matrix so add a small
# value for the last diagonal element
q = rep(0,3);
A = cbind(X*y,y);   # X*y not matrix multiplication. 
b = rep(1,N)
result = solve.QP(Q,q,t(A),b);  # painless QP result from package!!
